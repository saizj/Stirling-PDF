...
public class RedactController {
    ...
    /**
     * Proves a redacted file no longer yields the content it was asked to remove, and repairs it
     * where it still does.
     *
     * <p>A redaction engine reports how many matches it processed, which is not the same as the
     * text being unrecoverable: glyphs it cannot rewrite, and text that lives outside the page
     * content stream, still come back on copy-paste. Pages that leak are rasterized; the rest stay
     * selectable. Skipped when the whole document is about to be rasterized anyway.
     */
    private TempFile verifyRedactedFile(
            TempFile redactedFile, RedactionTarget target, Boolean convertToImage)
            throws IOException {
        if (Boolean.TRUE.equals(convertToImage)) {
            return redactedFile;
        }

        // Written to a fresh file rather than back over its own source: PDFBox reads lazily, so
        // saving in place can race the parse it is still feeding from.
        TempFile verified = tempFileManager.createManagedTempFile(".pdf");
        try (PDDocument document = pdfDocumentFactory.load(redactedFile.getFile())) {
            redactionVerificationService.secure(document, target);
            document.save(verified.getFile());
        } catch (Exception e) {
            verified.close();
            throw e;
        }
    ...
    public ResponseEntity<Resource> redactPdf(@ModelAttribute RedactPdfRequest request) {
        ...
        // What redaction was asked to remove, so both paths below can be held to the same
        // promise: the engine reporting success is not the same as the text being gone.
        RedactionTarget redactionTarget =
                RedactionTarget.ofText(
                        useRegex ? List.of() : terms,
                        useRegex ? terms : List.of(),
                        wholeWordSearchBool);
            ...
            try (TempFile tempInput = tempFileManager.createManagedTempFile(".pdf")) {
                ...
                try {
                    ...
                    try {
                        ...
                        return WebResponseUtils.pdfFileToWebResponse(verified, filename);
                    } finally {
